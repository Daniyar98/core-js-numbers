...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const firstVector = Math.sqrt(x1 * x1 + y1 * y1);
  const secondVector = Math.sqrt(x2 * x2 + y2 * y2);
  const angle = vectors / (firstVector * secondVector);
  const radius = Math.acos(angle);
  return radius;
}
